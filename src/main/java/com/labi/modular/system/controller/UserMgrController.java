...
import com.labi.common.constant.state.ManagerStatus;
import com.labi.common.constant.tips.Tip;
...
import com.labi.modular.system.factory.UserFactory;
import com.labi.modular.system.model.User;
import com.labi.modular.system.transfer.UserDto;
import com.labi.modular.system.warpper.UserWarpper;
...
public class UserMgrController extends BaseController {
    ...
    @Resource
    private LabiProperties labiProperties;
    ...
    public Object list(@RequestParam(required = false) String name, @RequestParam(required = false) String beginTime, @RequestParam(required = false) String endTime,
                       @RequestParam(required = false) Integer deptid, @RequestParam(required = false) String ifCustomer) {
        if (ShiroKit.isAdmin()) {
            List<Map<String, Object>> users = userMapper.selectUsers(null, name, beginTime, endTime, deptid, ifCustomer);
            return new UserWarpper(users).warp();
        } else {
            DataScope dataScope = new DataScope(ShiroKit.getDeptDataScope());
            List<Map<String, Object>> users = userMapper.selectUsers(dataScope, name, beginTime, endTime, deptid, ifCustomer);
            return new UserWarpper(users).warp();
        }
    }
    ...
    public Tip add(@Valid UserDto user, BindingResult result) {
        ...
        user.setIfCustomer(Const.FALSE);
     ...
     */
    @Permission(Const.ADMIN_NAME)
    @RequestMapping("/user_customer_type_edit/{userId}")
    public String addUserCustomerTypeEdit(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new BussinessException(BizExceptionEnum.REQUEST_NULL);
        }
        assertAuth(userId);
        User user = this.userMapper.selectById(userId);
        model.addAttribute(user);
        return PREFIX + "user_customer_type_edit.html";
    }
    /**
     * 设置客服
     */
    @RequestMapping("/customer")
    @BussinessLog(value = "设置客服", key = "userId", dict = Dict.UserDict)
    @ResponseBody
    public Tip setCustomer(@RequestParam Integer userId,String customerTypes) {
        if (ToolUtil.isEmpty(userId)) {
            throw new BussinessException(BizExceptionEnum.REQUEST_NULL);
        }
        assertAuth(userId);
        String ifCustomer;
        if(StringUtils.isNotBlank(customerTypes)){
            ifCustomer = Const.TRUE;
        } else {
            ifCustomer = Const.FALSE;
        }
        this.userMapper.setCustomer(userId, ifCustomer,customerTypes);
        return SUCCESS_TIP;
    }
    ...
    public String upload(@RequestPart("file") MultipartFile picture) {
        ...
        try {
            String fileSavePath = labiProperties.getFileUploadPath();
