 ...
 */
@Controller
@RequestMapping("/prodCatRecommend")
public class ProdCatRecommendController extends BaseController {

    private String PREFIX = "/biz/prodCatRecommend/";
    ...
    @RequestMapping("")
    public String index() {
        return PREFIX + "prodCatRecommend.html";
    }

    /**
     * 跳转到添加商品分类推荐
     */
    @RequestMapping("/prodCatRecommend_add")
    public String prodCatAdd(Model model) {
        ...
        wrapper.eq("if_recommend", Const.FALSE);
        ...
        return PREFIX + "prodCatRecommend_add.html";
    }
    ...
    public Object list(String catId) {
        ...
        wrapper.eq("if_recommend", Const.TRUE);
     ...
     * 跳转到修改商品分类管理
     */
    @RequestMapping("/prodCatRecommend_update/{prodCatId}")
    public String prodCatUpdate(@PathVariable Integer prodCatId, Model model) {
        ...
        wrapper.eq("status", Const.ACTIVE);//查询状态为A的数据
        wrapper.eq("parent_id", Const.UNKNOWN);
        wrapper.andNew("if_recommend={0} or id={1}", Const.FALSE, prodCatId);
        Integer depid = ShiroKit.getUser().getDeptId();//所属公司
        if (depid.intValue() != Const.IS_ADMIN_FLG) {
            wrapper.eq("type", depid);
        }
        ...
        return PREFIX + "prodCatRecommend_edit.html";
    }
    ...
    public Object update(ProdCat prodCat) {
        prodCat.setIfRecommend(Const.TRUE);
    ...
    public Object delete(Integer id) {
        ...
        prodCat.setId(id);
        prodCat.setOprtime(new Date());
        prodCat.setIfRecommend(Const.FALSE);
        prodCatService.updateById(prodCat);
        return SUCCESS_TIP;
    }
}
