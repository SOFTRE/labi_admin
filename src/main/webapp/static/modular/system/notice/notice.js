...
Notice.initColumn = function () {
    return [
        ...
        {title: '通知类型', field: 'type', align: 'center', valign: 'middle', sortable: true,
...
Notice.openAddNotice = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Labi.ctxPath + '/notice/notice_add'
    });
...
Notice.openNoticeDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Labi.ctxPath + '/notice/notice_update/' + Notice.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Notice.delete = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(Labi.ctxPath + "/notice/delete", function (data) {
                Labi.success("删除成功!");
        ...
        Labi.confirm("是否删除通知 " + Notice.seItem.title + "?", operation);
    }
};
