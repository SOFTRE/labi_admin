...
MenuInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Labi.ctxPath + "/menu/add", function (data) {
        Labi.success("添加成功!");
...
MenuInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Labi.ctxPath + "/menu/edit", function (data) {
        Labi.success("修改成功!");
        window.parent.Menu.table.refresh();
        MenuInfoDlg.close();
    }, function (data) {
...
MenuInfoDlg.showMenuSelectTree = function () {
    Labi.showInputTree("pcodeName", "pcodeTreeDiv", 15, 34);
};

$(function () {
    Labi.initValidator("menuInfoForm", MenuInfoDlg.validateFields);
