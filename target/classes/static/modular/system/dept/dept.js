...
Dept.openAddDept = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Labi.ctxPath + '/dept/dept_add'
    });
...
Dept.openDeptDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Labi.ctxPath + '/dept/dept_update/' + Dept.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Dept.delete = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(Labi.ctxPath + "/dept/delete", function () {
                Labi.success("删除成功!");
                Dept.table.refresh();
            }, function (data) {
        ...
        Labi.confirm("是否刪除该公司?", operation);
    }
};
