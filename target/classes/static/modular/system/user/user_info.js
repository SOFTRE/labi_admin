...
UserInfoDlg.addSubmit = function () {
    ...
    if (!this.validatePwd()) {
        Labi.error("两次密码输入不一致");
        return;
    }

    //提交信息
    var ajax = new $ax(Labi.ctxPath + "/mgr/add", function (data) {
        Labi.success("添加成功!");
...
UserInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Labi.ctxPath + "/mgr/edit", function (data) {
        Labi.success("修改成功!");
...
UserInfoDlg.chPwd = function () {
    var ajax = new $ax(Labi.ctxPath + "/mgr/changePwd", function (data) {
        Labi.success("修改成功!");
    }, function (data) {
        Labi.error("修改失败!" + data.responseJSON.message + "!");
    });
 ...
 * 提交修改 客服业务类型
 */
UserInfoDlg.addCustomerTypeSubmit = function () {
    var arr = [];
    $('.customer-type:checked').each(function (i, obj) {
        arr.push(obj.value);
    })
    //提交信息
    ...
    ajax.set("customerTypes", arr.join());
    ajax.start();
};
...
$(function () {
    Labi.initValidator("userInfoForm", UserInfoDlg.validateFields);
    ...
    avatarUp.init();
    //radio初始化
    $('.customer-type').iCheck({
        checkboxClass: 'icheckbox_square-green',
        radioClass: 'iradio_square-green',
        increaseArea: '20%' // optional
    });
    if ($('#customer_type').length > 0) {
        var customerTypes = $('#customer_type').val();
        $('.customer-type').each(function (i, obj) {
            if(customerTypes.indexOf(obj.value)>-1){
                $(obj).iCheck('check');
            }
        })
    }
