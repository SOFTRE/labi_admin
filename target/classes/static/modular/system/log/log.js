...
OptLog.initColumn = function () {
    return [
        ...
        {title: '类名', field: 'classname', align: 'center', valign: 'middle', sortable: true,visible: false,},
        {title: '方法名', field: 'method', align: 'center', valign: 'middle', sortable: true,visible: false,},
...
OptLog.detail = function () {
    if (this.check()) {
        var ajax = new $ax(Labi.ctxPath + "/log/detail/" + this.seItem.id, function (data) {
            Labi.infoDetail("日志详情", data.regularMessage);
        }, function (data) {
            Labi.error("获取详情失败!");
        });
        ajax.start();
...
OptLog.delLog = function () {
    Labi.confirm("是否清空所有日志?",function(){
        var ajax = Labi.baseAjax("/log/delLog","清空日志");
        ajax.start();
