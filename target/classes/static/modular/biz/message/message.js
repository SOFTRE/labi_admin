...
Message.openAddMessage = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Labi.ctxPath + '/message/message_add'
    });
...
Message.openMessageDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Labi.ctxPath + '/message/message_update/' + Message.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Message.delete = function () {
    if (this.check()) {
    	var operation = function(){
	        var ajax = new $ax(Labi.ctxPath + "/message/delete", function (data) {
	            Labi.success("删除成功!");
