    ...
    BootstrapTable.prototype.initData = function () {
        ...
        if (that.options.treeView && this.data.length > 0) {
            ...
            var roots = $.grep(this.data, function (row, index) {
                return row.depth == that.options.treeRootLevel;
            });
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (column.checkbox || column.radio) {
                    ...
                    if (that.options.treeView && column.field == that.options.treeField) {
                        var indent = item.depth == that.options.depth ? '' : sprintf('<span style="margin-left: %spx;"></span>', (item.depth - that.options.treeRootLevel) * 45);
